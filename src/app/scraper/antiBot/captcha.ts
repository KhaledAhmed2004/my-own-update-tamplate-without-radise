...
export interface IHumanReadableError {
  code: ScraperErrorCode;
  message: string;
  messageBn: string; // Bangla message
  suggestion: string;
  suggestionBn: string; // Bangla suggestion
  details?: {
    captchaType?: string;
    methodsTried?: string[];
    url?: string;
 ...
 */
const ERROR_MESSAGES: Record<ScraperErrorCode, Omit<IHumanReadableError, 'code' | 'details'>> = {
  CAPTCHA_DETECTED: {
...
export interface ICaptchaDetectionResult {
  detected: boolean;
  type?: 'recaptcha' | 'hcaptcha' | 'cloudflare' | 'amazon' | 'unknown';
  confidence: number; // 0-100
}

export interface IBlockingDetectionResult {
  blocked: boolean;
  reason?: string;
  statusCode?: number;
}
 ...
 */
export const detectCaptcha = (html: string): ICaptchaDetectionResult => {
  if (!html) {
  ...
  let matchCount = 0;
  let detectedType: ICaptchaDetectionResult['type'];
  ...
  for (const selector of CAPTCHA_SELECTORS) {
    ...
    if (
      lowerHtml.includes(selectorPattern) ||
 ...
 */
export const detectBlocking = (html: string, statusCode?: number): IBlockingDetectionResult => {
  // Check HTTP status code
  if (statusCode) {
    const blockedCodes: Record<number, string> = {
  ...
  const lowerHtml = html.toLowerCase();
  const blockedIndicators: Record<string, string> = {
...
export const detectBlocked = (html: string, statusCode?: number): boolean => {
  return detectBlocking(html, statusCode).blocked;
};
...
export const getCaptchaType = (
  html: string
): 'recaptcha' | 'hcaptcha' | 'cloudflare' | 'amazon' | 'unknown' | null => {
  if (!html) return null;
 ...
 */
export const isRetryableError = (error: any, html?: string): boolean => {
  ...
  if (html && detectBlocked(html)) {
    return true;
  }

  // CAPTCHA might be retryable with different approach
  if (html) {
    const captchaResult = detectCaptcha(html);
...
/**
 * Smart CAPTCHA detection result
 */
export interface ISmartDetectionResult {
  isBlocked: boolean;
  reason?: string;
  captchaType?: string;
  confidence: number;
 ...
 */
export function smartDetectBlocking(html: string): ISmartDetectionResult {
  const result: ISmartDetectionResult = {
    isBlocked: false,
    confidence: 0,
    pageSize: html?.length || 0,
  ...
  if (!html || html.length < 100) {
    return {
      ...result,
      isBlocked: true,
      reason: 'Empty or too small response',
      confidence: 100,
    };
  }
  ...
  result.hasProductContent = PRODUCT_INDICATORS.some(indicator =>
  ...
  if (html.length > 50000 && result.hasProductContent) {
    return {
      ...
      isBlocked: false,
      confidence: 0,
      reason: 'Large page with product content - accepting',
    };
  }
  ...
  if (captchaInfo.detected && captchaInfo.confidence > 50 && html.length < 50000) {
    return {
      ...result,
      isBlocked: true,
      reason: `CAPTCHA detected (${captchaInfo.type})`,
