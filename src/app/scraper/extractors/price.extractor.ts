...
// ==================== 25-Layer Price Selectors ====================
// Priority order with confidence scores (higher = more reliable)
// Supports: Amazon (1-5), eBay (6-9), AliExpress (10-14), Generic (15-25)

interface IPriceSelector {
  selector: string;
  confidence: number;
  site?: string; // Optional: site-specific selector
}
 ...
 */
function parsePrice(text: string): IExtractedPrice | null {
  if (!text) return null;
 ...
 */
function detectSiteType(url: string): 'amazon' | 'ebay' | 'aliexpress' | 'walmart' | 'generic' {
...
// Export for use in other extractors
export { detectSiteType, PRICE_SELECTORS_15_LAYER };
